...
const AIReadinessSection = ({ postId, onCommit, isCommitting, commitStatus }) => {
  ...
  const scoreItems = [
    { key: 'readability', label: __('Readability', 'ai-readability-optimizer') },
    { key: 'structure', label: __('Structure', 'ai-readability-optimizer') },
    { key: 'freshness', label: __('Freshness', 'ai-readability-optimizer') },
    { key: 'citationReadiness', label: __('Citation Ready', 'ai-readability-optimizer') },
    { key: 'aiVisibility', label: __('AI Visibility', 'ai-readability-optimizer') },
  ];
    ...
    <div className="rain-os-ai-readiness">
      ...
      >
        <span className="rain-os-ai-readiness-title">
          {__('AI Readiness', 'ai-readability-optimizer')}
        </span>
        ...
        <>
          ...
          >
            {isCommitting ? (
              <>
                ...
                {__('Committing...', 'ai-readability-optimizer')}
              </>
            ) : (
              __('Commit Content', 'ai-readability-optimizer')
            )}
          </button>
          ...
          {isLoading ? (
            <div className="rain-os-loading" style={{ padding: '20px' }}>
              ...
              <span>{__('Loading scores...', 'ai-readability-optimizer')}</span>
            </div>
          ) : error ? (
            ...
            <div style={{ color: '#64748b', fontSize: '12px', textAlign: 'center', padding: '12px' }}>
              {__('Commit content to get AI readiness scores', 'ai-readability-optimizer')}
            </div>
          )}
        </>
      )}
