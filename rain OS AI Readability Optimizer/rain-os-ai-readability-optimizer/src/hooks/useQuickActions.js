...
export const useQuickActions = () => {
  ...
  const suggestTitles = useCallback(async (content, currentTitle) => {
    ...
    try {
      const response = await apiFetch({
        path: '/ai-readability/v1/quick-action',
        method: 'POST',
        data: {
  ...
  const rewriteSelection = useCallback(async (selectedText) => {
    if (!selectedText || selectedText.trim().length < 10) {
      setResults({
        rewrite: {
          original: __('Please select some text first', 'ai-readability-optimizer'),
          improved: __('Select at least 10 characters of text to rewrite.', 'ai-readability-optimizer'),
        },
      });
      return;
    }
