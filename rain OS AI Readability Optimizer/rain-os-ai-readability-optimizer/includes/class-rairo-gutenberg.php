...
class RAIRO_Gutenberg {
    ...
    public function register_rest_routes() {
        register_rest_route( 'ai-readability/v1', '/analyze', array(
            'methods'             => 'POST',
            'callback'            => array( $this, 'handle_analyze' ),
            'permission_callback' => array( $this, 'check_edit_permission' ),
        ) );

        register_rest_route( 'ai-readability/v1', '/normalize', array(
            'methods'             => 'POST',
            'callback'            => array( $this, 'handle_normalize' ),
            'permission_callback' => array( $this, 'check_edit_permission' ),
        ) );

        register_rest_route( 'ai-readability/v1', '/normalize/status/(?P<task_id>[a-zA-Z0-9_-]+)', array(
            'methods'             => 'GET',
            'callback'            => array( $this, 'handle_normalize_status' ),
            'permission_callback' => array( $this, 'check_edit_permission' ),
        ) );

        register_rest_route( 'ai-readability/v1', '/ai-scores/(?P<post_id>\d+)', array(
            'methods'             => 'GET',
            'callback'            => array( $this, 'handle_get_ai_scores' ),
            'permission_callback' => array( $this, 'check_edit_permission' ),
        ) );

        register_rest_route( 'ai-readability/v1', '/history/(?P<post_id>\d+)', array(
            'methods'             => 'GET',
            'callback'            => array( $this, 'handle_get_history' ),
            'permission_callback' => array( $this, 'check_edit_permission' ),
        ) );

        register_rest_route( 'ai-readability/v1', '/quick-action', array(
            'methods'             => 'POST',
            'callback'            => array( $this, 'handle_quick_action' ),
            'permission_callback' => array( $this, 'check_edit_permission' ),
        ) );

        register_rest_route( 'ai-readability/v1', '/backend-analysis/(?P<post_id>\d+)', array(
            'methods'             => 'GET',
    ...
    public function handle_get_ai_scores( $request ) {
        $post_id = absint( $request->get_param( 'post_id' ) );
        $scores = get_post_meta( $post_id, '_ai_readability_ai_scores', true );
