        ...
        <ChartCard title="Overall Score" period={periodLabel} className="animate-in-delay-1">
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            {[
              ...
              <div key={pillarIdx} style={{ padding: '14px', backgroundColor: 'var(--bg-tertiary)', borderRadius: '8px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                  <span style={{ fontSize: '13px', fontWeight: 600, color: pillar.colors[0] }}>{pillar.name}</span>
                  <span style={{ fontSize: '16px', fontWeight: 700, color: pillar.colors[0] }}>{pillar.score}</span>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {pillar.subcategories.map((sub, subIdx) => (
                    <div key={subIdx}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                        <span style={{ fontSize: '11px', color: 'var(--text-secondary)' }}>{sub.name}</span>
                        <span style={{ fontSize: '12px', fontWeight: 600, color: pillar.colors[subIdx] }}>{sub.value}</span>
                      </div>
