...
const RESPONSE_SCHEMA = {
  ...
  ai_readability_detail: {
    structuralClarity: 0,
    answerFirstFormatting: 0,
    semanticPrecision: 0,
    contextSufficiency: 0,
    sectionConceptIsolation: 0,
  },
  ...
  rag_readiness_detail: {
    ...
    semanticMapping: 0,
    narrativeNuance: 0,
    hierarchicalFormatting: 0,
    explicitQaStructures: 0,
    authoritySignals: 0,
  },
  phase2_sub_scores: {
    sectionConceptIsolation: 0,
    instructionDeterminism: 0,
    retrievalAnswerability: 0,
    semanticRedundancyScore: 0,
    socialProofMarkup: 0,
    contentChunkingQuality: 0,
    informationGainScore: 0,
    citationReadiness: 0,
    entityLinkability: 0,
    topicalDepthScore: 0,
    queryAlignmentScore: 0,
    multimodalReadiness: 0,
    productVariantCoverage: 0,
    merchantIdentityClarity: 0,
  },
  subScores: [],
...
For this module, score AI Readability as Documentation Structure (navigation, TOC, getting started), Digital Authority as Documentation Authority (API completeness, error coverage, versioning).`
...
RAG Readiness is scored at 10% weight and always included. It measures RAG-system retrieval quality.`;
  ...
  if (parsed.phase2_sub_scores?.semanticRedundancyScore !== undefined) {
    const raw_redundancy = clamp(parsed.phase2_sub_scores.semanticRedundancyScore);
  ...
  const moduleWeights = module === 'product_sellers'
    ? { ar: 0.2, da: 0.15, cr: 0.15, pd: 0.5, rr: 0.1 }
    : module === 'developers'
    ? { ar: 0.32, da: 0.32, cr: 0.26, pd: 0.0, rr: 0.1 }
    : module === 'local_business'
    ? { ar: 0.27, da: 0.36, cr: 0.27, pd: 0.0, rr: 0.1 }
    : { ar: 0.36, da: 0.27, cr: 0.27, pd: 0.0, rr: 0.1 }; // general
  ...
  const subScores: SubScore[] = [
    {
      category: 'structuralClarity',
      score: clamp(parsed.ai_readability_detail?.structuralClarity || 0),
      label: 'Structural Clarity',
    },
    {
      category: 'answerFirstFormatting',
      score: clamp(parsed.ai_readability_detail?.answerFirstFormatting || 0),
      label: 'Answer-First Formatting',
    },
    {
      category: 'semanticPrecision',
      score: clamp(parsed.ai_readability_detail?.semanticPrecision || 0),
      label: 'Semantic Precision',
    },
    {
      category: 'contextSufficiency',
      score: clamp(parsed.ai_readability_detail?.contextSufficiency || 0),
      label: 'Context Sufficiency',
    },
    {
      category: 'citationSignals',
      score: clamp(parsed.digital_authority_detail?.citationSignals || 0),
      label: 'Citation Signals',
    },
    {
      category: 'entityClarity',
      score: clamp(parsed.digital_authority_detail?.entityClarity || 0),
      label: 'Entity Clarity',
    },
    {
      category: 'topicalAuthority',
      score: clamp(parsed.digital_authority_detail?.topicalAuthority || 0),
      label: 'Topical Authority',
    },
    {
      category: 'freshnessSignals',
      score: clamp(parsed.digital_authority_detail?.freshnessSignals || 0),
      label: 'Freshness Signals',
    },
    {
      category: 'socialProofMarkup',
      score: clamp(parsed.digital_authority_detail?.socialProofMarkup || 0),
      label: 'Social Proof Markup',
    },
    {
      category: 'callToActionClarity',
      score: clamp(parsed.conversion_readiness_detail?.callToActionClarity || 0),
      label: 'CTA Clarity',
    },
    {
      category: 'valueProposition',
      score: clamp(parsed.conversion_readiness_detail?.valueProposition || 0),
      label: 'Value Proposition',
    },
    {
      category: 'productDiscoverability',
      score: clamp(parsed.pillarScores?.productDiscoverability || 0),
      label: 'Product Discoverability',
    },
    {
      category: 'ragReadiness',
      score: clamp(parsed.pillarScores?.ragReadiness || 0),
      label: 'RAG Readiness',
    },
    {
      category: 'informationDensity',
      score: clamp(parsed.rag_readiness_detail?.informationDensity || 0),
      label: 'Information Density',
    },
    {
      category: 'semanticMapping',
      score: clamp(parsed.rag_readiness_detail?.semanticMapping || 0),
      label: 'Semantic Mapping',
    },
    {
      category: 'narrativeNuance',
      score: clamp(parsed.rag_readiness_detail?.narrativeNuance || 0),
      label: 'Narrative Nuance',
    },
    {
      category: 'hierarchicalFormatting',
      score: clamp(parsed.rag_readiness_detail?.hierarchicalFormatting || 0),
      label: 'Hierarchical Formatting',
    },
    {
      category: 'explicitQaStructures',
      score: clamp(parsed.rag_readiness_detail?.explicitQaStructures || 0),
      label: 'Explicit Q&A Structures',
    },
    {
      category: 'authoritySignals',
      score: clamp(parsed.rag_readiness_detail?.authoritySignals || 0),
...
export const generateDescription = async (content: string): Promise<{ description: string }> => {
  const truncated = truncateContentUpfront(content);
  const raw = await callGeminiSimple(
    truncated,
...
export const rewriteSentence = async (sentence: string): Promise<{ rewritten: string }> => {
...
export async function rewriteDocumentForAI(
  ...
  const moduleContext = module === 'developers'
    ...
    : module === 'product_sellers'
    ? 'This is product content. Priorities: lead with the primary benefit, make pricing/variants/availability explicit, add comparison context, ensure strong purchase CTAs, use specific claims.'
    : 'This is content marketing or editorial content. Priorities: answer-first paragraphs, scannable headings, concise active sentences, strong CTA placement, citation-ready statements.';
