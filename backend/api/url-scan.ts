...
// URL scans count as one usage credit (same as a content analysis).
...
function applyTechnicalAdjustments(base: number, signals: Record<string, any>):
number {
let adj = 0;
...
return res.status(422).json({ error: 'fetch_failed', message: msg } as
ApiError);
}
...
const adjustedScore = applyTechnicalAdjustments(gemini.overallScore,
scan.signals);
const result = {
...gemini,
